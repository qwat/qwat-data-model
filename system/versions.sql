...
Usage :
select * from qwat_sys.version_dump('1.0.6-1+build-623');
select (qwat_sys.version_dump('1.0.6-1+build-623')).*;
select (qwat_sys.version_dump('1.0.6-1+build-623')).major;
*/
create type qwat_sys.version_result as (
...
create or replace function qwat_sys.version_dump(version varchar) returns qwat_sys.version_result as $$
select
