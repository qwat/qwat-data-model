/* Audit */
SELECT qwat_sys.audit_table('qwat_dr.annotationline');
SELECT qwat_sys.audit_table('qwat_dr.annotationpoint');
SELECT qwat_sys.audit_table('qwat_dr.constructionpoint');
SELECT qwat_sys.audit_table('qwat_dr.dimension_distance');
SELECT qwat_sys.audit_table('qwat_dr.dimension_orientation');

SELECT qwat_sys.audit_table('qwat_od.distributor');
SELECT qwat_sys.audit_table('qwat_od.district');
SELECT qwat_sys.audit_table('qwat_od.hydrant');
SELECT qwat_sys.audit_table('qwat_od.installation');
SELECT qwat_sys.audit_table('qwat_od.pressurecontrol');
SELECT qwat_sys.audit_table('qwat_od.pump');
SELECT qwat_sys.audit_table('qwat_od.source');
SELECT qwat_sys.audit_table('qwat_od.treatment');
SELECT qwat_sys.audit_table('qwat_od.tank');
SELECT qwat_sys.audit_table('qwat_od.chamber');
SELECT qwat_sys.audit_table('qwat_od.leak');
SELECT qwat_sys.audit_table('qwat_od.meter');
SELECT qwat_sys.audit_table('qwat_od.pipe');
SELECT qwat_sys.audit_table('qwat_od.pressurezone');
SELECT qwat_sys.audit_table('qwat_od.printmap');
SELECT qwat_sys.audit_table('qwat_od.protectionzone');
SELECT qwat_sys.audit_table('qwat_od.samplingpoint');
SELECT qwat_sys.audit_table('qwat_od.subscriber');
SELECT qwat_sys.audit_table('qwat_od.subscriber_reference');
SELECT qwat_sys.audit_table('qwat_od.surveypoint');
SELECT qwat_sys.audit_table('qwat_od.valve');

SELECT qwat_sys.audit_table('qwat_vl.cistern');
SELECT qwat_sys.audit_table('qwat_vl.hydrant_provider');
SELECT qwat_sys.audit_table('qwat_vl.hydrant_material');
SELECT qwat_sys.audit_table('qwat_vl.leak_cause');;
SELECT qwat_sys.audit_table('qwat_vl.overflow');
SELECT qwat_sys.audit_table('qwat_vl.pipe_function');
SELECT qwat_sys.audit_table('qwat_vl.pipe_installmethod');
SELECT qwat_sys.audit_table('qwat_vl.pipe_material');
SELECT qwat_sys.audit_table('qwat_vl.pipe_protection');
SELECT qwat_sys.audit_table('qwat_vl.precision');
SELECT qwat_sys.audit_table('qwat_vl.pressurecontrol_type');
SELECT qwat_sys.audit_table('qwat_vl.protectionzone_type');
SELECT qwat_sys.audit_table('qwat_vl.pump_type');
SELECT qwat_sys.audit_table('qwat_vl.remote_type');
SELECT qwat_sys.audit_table('qwat_vl.source_quality');
SELECT qwat_sys.audit_table('qwat_vl.source_type');
SELECT qwat_sys.audit_table('qwat_vl.status');
SELECT qwat_sys.audit_table('qwat_vl.subscriber_type');
SELECT qwat_sys.audit_table('qwat_vl.survey_type');
SELECT qwat_sys.audit_table('qwat_vl.tank_firestorage');
SELECT qwat_sys.audit_table('qwat_vl.valve_function');
SELECT qwat_sys.audit_table('qwat_vl.valve_maintenance');
SELECT qwat_sys.audit_table('qwat_vl.valve_actuation');
SELECT qwat_sys.audit_table('qwat_vl.valve_type');
SELECT qwat_sys.audit_table('qwat_vl.visible');
