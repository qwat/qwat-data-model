...
CREATE OR REPLACE FUNCTION qwat_app.fn_node_set_type(_node_id integer) RETURNS void AS
$BODY$
    DECLARE
        _pipeitem     record;
        _pipe_id      integer;
...
$BODY$
LANGUAGE plpgsql;
COMMENT ON FUNCTION qwat_app.fn_node_set_type(integer) IS 'Set the orientation and type for a node. If three pipe arrives at the node: intersection. If one pipe: end. If two: depends on characteristics of pipe: year (is different), material (and year), diameter(and material/year)';
...
/* reset all node type */
CREATE OR REPLACE FUNCTION qwat_app.fn_node_set_type( _node_ids integer[] DEFAULT NULL ) RETURNS void AS
$BODY$
	DECLARE
		node record;
		_node_id integer;
	BEGIN
		IF _node_ids IS NULL THEN
			FOR node IN (SELECT id FROM qwat_od.node ORDER BY id) LOOP
				PERFORM qwat_app.fn_node_set_type(node.id);
			END LOOP;
		ELSE
			FOREACH _node_id IN ARRAY _node_ids LOOP
				PERFORM qwat_app.fn_node_set_type(_node_id);
			END LOOP;
		END IF;
	END;
$BODY$
LANGUAGE plpgsql;
COMMENT ON FUNCTION qwat_app.fn_node_set_type( _node_ids integer[] ) IS 'Set the type and orientation for node. If three pipe arrives at the node: intersection. If one pipe: end. If two: depends on characteristics of pipe: year (is different), material (and year), diameter(and material/year)';
