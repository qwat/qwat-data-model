...
CREATE OR REPLACE FUNCTION qwat_app.if_modified_func() RETURNS TRIGGER AS $body$
DECLARE
    audit_row qwat_sys.logged_actions%ROWTYPE;
...
BEGIN
    ...
    IF NOT (TG_WHEN IN ('AFTER' , 'INSTEAD OF')) THEN
        RAISE EXCEPTION 'qwat_app.if_modified_func() may only run as an AFTER trigger';
    END IF;
    ...
    IF (TG_OP = 'UPDATE' AND TG_LEVEL = 'ROW') THEN
        ...
        IF audit_row.changed_fields = hstore('') THEN
            ...
            RAISE WARNING '[qwat_app.if_modified_func] - Trigger detected NULL hstore. ending';
            RETURN NULL;
        END IF;
  ...
  RETURN NULL;
    ...
    ELSE
        RAISE EXCEPTION USING MESSAGE = '[qwat_app.if_modified_func] - Trigger func added as trigger for unhandled case: '||TG_OP||', '||TG_LEVEL;
        RETURN NEW;
    END IF;
...
COMMENT ON FUNCTION qwat_app.if_modified_func() IS $body$
...
CREATE OR REPLACE FUNCTION qwat_app.audit_table(target_table regclass, audit_rows BOOLEAN, audit_query_text BOOLEAN, ignored_cols text[]) RETURNS void AS $body$
DECLARE
...
BEGIN
    ...
    IF audit_rows THEN
        ...
        _q_txt = 'CREATE TRIGGER audit_trigger_row AFTER INSERT OR UPDATE OR DELETE ON ' ||
                 target_table::text ||
                 ' FOR EACH ROW EXECUTE PROCEDURE qwat_app.if_modified_func(' ||
    ...
    _q_txt = 'CREATE TRIGGER audit_trigger_stm AFTER ' || stm_targets || ' ON ' ||
             target_table ||
             ' FOR EACH STATEMENT EXECUTE PROCEDURE qwat_app.if_modified_func('||
...
COMMENT ON FUNCTION qwat_app.audit_table(regclass, BOOLEAN, BOOLEAN, text[]) IS $body$
...
CREATE OR REPLACE FUNCTION qwat_app.audit_table(target_table regclass, audit_rows BOOLEAN, audit_query_text BOOLEAN) RETURNS void AS $body$
SELECT qwat_app.audit_table($1, $2, $3, ARRAY[]::text[]);
$body$ LANGUAGE SQL;

-- And provide a convenience call wrapper for the simplest case
-- of row-level logging with no excluded cols and query logging enabled.
--
CREATE OR REPLACE FUNCTION qwat_app.audit_table(target_table regclass) RETURNS void AS $body$
SELECT qwat_app.audit_table($1, BOOLEAN 't', BOOLEAN 't');
$body$ LANGUAGE 'sql';

COMMENT ON FUNCTION qwat_app.audit_table(regclass) IS $body$
Add auditing support to the given table. Row-level changes will be logged with full client query text. No cols are ignored.
$body$;

CREATE OR REPLACE FUNCTION qwat_app.unaudit_table(target_table regclass) RETURNS void AS $body$
BEGIN
    EXECUTE 'DROP TRIGGER IF EXISTS audit_trigger_row ON ' || target_table::text;
    EXECUTE 'DROP TRIGGER IF EXISTS audit_trigger_stm ON ' || target_table::text;
END;
...
CREATE OR REPLACE FUNCTION qwat_app.replay_event(pevent_id int) RETURNS void AS $body$
DECLARE
  query text;
BEGIN
    with
...
COMMENT ON FUNCTION qwat_app.replay_event(int) IS $body$
...
CREATE OR REPLACE FUNCTION qwat_app.audit_view(target_view regclass, audit_query_text BOOLEAN, ignored_cols text[], uid_cols text[]) RETURNS void AS $body$
DECLARE
...
COMMENT ON FUNCTION qwat_app.audit_view(regclass, BOOLEAN, text[], text[]) IS $body$
...
Example:
  SELECT qwat_app.audit_view('view_schema.view_name', 'true'::BOOLEAN, 'field_to_ignore'::text[], 'obj_id'::text[])
$body$;

CREATE OR REPLACE FUNCTION qwat_app.unaudit_view(target_view regclass) RETURNS void AS $body$
BEGIN
    EXECUTE 'DROP TRIGGER IF EXISTS audit_trigger_row ON ' || target_view::text;
    EXECUTE 'DROP TRIGGER IF EXISTS audit_trigger_stm ON ' || target_view::text;
END;
