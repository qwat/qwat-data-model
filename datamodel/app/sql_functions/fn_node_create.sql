...
CREATE OR REPLACE FUNCTION qwat_app.fn_node_create( _point geometry, deactivate_node_add_pipe_vertex boolean = FALSE, status integer = -1, distributors integer[] = '{}'::integer[] ) RETURNS integer AS
$BODY$
    DECLARE
        _node_id integer;
    BEGIN
...
$BODY$
LANGUAGE plpgsql;
COMMENT ON FUNCTION qwat_app.fn_node_create(geometry, boolean, integer, integer[]) IS 'Returns the node for a given geometry (point). If node does not exist, create it.';
