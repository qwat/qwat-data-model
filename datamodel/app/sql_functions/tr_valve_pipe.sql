...
CREATE OR REPLACE FUNCTION qwat_app.tn_pipe_delete_valve(_valve_id integer)
  RETURNS void AS
$BODY$
BEGIN
...
CREATE OR REPLACE FUNCTION qwat_app.tn_pipe_update_valve(_valve_id integer)
  RETURNS void AS
$BODY$
	BEGIN
		UPDATE qwat_od.pipe
...
$BODY$
    DECLARE
        r record;
    BEGIN
        UPDATE qwat_od.valve SET fk_pipe = qwat_app.tn_pipe_get_id(geometry) WHERE fk_pipe = OLD.id OR ST_Distance(geometry, OLD.geometry) < 1e-4;
        ...
        LOOP
            PERFORM qwat_app.tn_valve_set_orientation(r.id);
        END LOOP;
...
$BODY$
	BEGIN
		PERFORM qwat_app.tn_pipe_update_valve(NEW.id);
		RETURN NEW;
	END;
$BODY$
LANGUAGE plpgsql;
...
$BODY$
	BEGIN
		IF TG_OP = 'DELETE' THEN
			PERFORM qwat_app.tn_pipe_delete_valve(OLD.id);
		END IF;
...
$BODY$
    BEGIN
        NEW.fk_pipe             := qwat_app.tn_pipe_get_id(NEW.geometry);
        NEW.fk_district         := qwat_app.tn_get_district(NEW.geometry);
        NEW.fk_pressurezone     := qwat_app.tn_get_pressurezone(NEW.geometry);
