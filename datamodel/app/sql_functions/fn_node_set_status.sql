...
CREATE OR REPLACE FUNCTION qwat_app.fn_node_get_status(_node_id integer, user_value integer = -1)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
	declare
...
$function$
;
COMMENT ON FUNCTION qwat_app.fn_node_get_status( _node_id integer, user_value integer ) IS 'Get the status of node regarding connected pipes. If one pipe: status of the pipe. If many: depends on priority of status of all pipes. If none: get value from user.';

-- Set status of a node regarding connected pipes
CREATE OR REPLACE FUNCTION qwat_app.fn_node_set_status(_node_id integer, user_value integer default -1)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
	declare
	    _status		integer;
	begin
    	_status := qwat_app.fn_node_get_status(_node_id, user_value); 	
...
$function$
;
COMMENT ON FUNCTION qwat_app.fn_node_set_status( _node_id integer, user_value integer ) IS 'Set the status of node regarding connected pipes. If one pipe: status of the pipe. If many: depends on priority of status of all pipes.';

-- Set status of each nodes
CREATE OR REPLACE FUNCTION qwat_app.fn_node_set_status(_node_ids integer[] DEFAULT NULL::integer[])
 RETURNS void
 LANGUAGE plpgsql
AS $function$
	DECLARE
	...
	BEGIN
		IF _node_ids IS NULL THEN
			FOR node IN (SELECT id FROM qwat_od.node ORDER BY id) LOOP
				PERFORM qwat_app.fn_node_set_status(node.id);
			END LOOP;
		ELSE
			FOREACH _node_id IN ARRAY _node_ids LOOP
				PERFORM qwat_app.fn_node_set_status(_node_id);
			END LOOP;
		END IF;
	END;
$function$
;
COMMENT ON FUNCTION qwat_app.fn_node_set_status( _node_ids integer[] ) IS 'Set the status of nodes regarding connected pipes.';
