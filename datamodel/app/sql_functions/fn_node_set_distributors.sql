...
CREATE OR REPLACE FUNCTION qwat_app.fn_node_set_distributors(_node_id integer)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
	declare
...
$function$
;
COMMENT ON FUNCTION qwat_app.fn_node_set_distributors( _node_id integer ) IS 'Set distributor(s) of node regarding connected pipes.';

CREATE OR REPLACE FUNCTION qwat_app.fn_node_set_distributors(_node_ids integer[] DEFAULT NULL::integer[])
 RETURNS void
 LANGUAGE plpgsql
AS $function$
	DECLARE
	...
	BEGIN
		IF _node_ids IS NULL THEN
			FOR node IN (SELECT id FROM qwat_od.node ORDER BY id) LOOP
				PERFORM qwat_app.fn_node_set_distributors(node.id);
			END LOOP;
		ELSE
			FOREACH _node_id IN ARRAY _node_ids LOOP
				PERFORM qwat_app.fn_node_set_distributors(_node_id);
			END LOOP;
		END IF;
	END;
$function$
;
COMMENT ON FUNCTION qwat_app.fn_node_set_distributors( _node_ids integer[] ) IS 'Set distributor(s) of nodes regarding connected pipes.';
