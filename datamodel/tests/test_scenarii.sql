...
VALUES (1, 2, 101, 101, 101, 1, 101, 101, 101, 101,
    st_setsrid('linestring(2730000 1340000 0,2830000 1440000 0,2930000 1540000 0)'::geometry, 2056),
    2016, 101, True);
    -- update that pipe
UPDATE qwat_od.pipe SET geometry = st_setsrid('linestring(2730000 1340000 0,2830000 1440000 0)'::geometry, 2056) WHERE geometry = st_setsrid('linestring(2730000 1340000 0,2830000 1440000 0,2930000 1540000 0)'::geometry, 2056);
    -- create a new one
INSERT INTO qwat_od.pipe (fk_node_a, fk_node_b,
    fk_function, fk_installmethod, fk_material, fk_distributor, fk_precision, fk_bedding,
    fk_status, fk_watertype, geometry, year, fk_protection,schema_force_visible)
VALUES (1, 2, 101, 101, 101, 1, 101, 101, 101, 101,
    st_setsrid('linestring(2830000 1440000 0,2930000 1540000 0)'::geometry, 2056),
    2016, 101, True);
...
INSERT INTO qwat_od.vw_element_part (year, _pipe_orientation, fk_part_type, fk_status, fk_distributor, fk_precision, geometry)
    SELECT 2016, 1.5, (SELECT id FROM qwat_vl.part_type WHERE value_fr = 'bouchon'), 101, '{1}', 101, st_setsrid('point(2530000 1140000 0)'::geometry, 2056);

-- create an introduction point
INSERT INTO qwat_vl.part_type (id, value_fr) VALUES (999, 'Point d''introduction');

INSERT INTO qwat_od.vw_element_part (year, _pipe_orientation, fk_part_type, fk_status, fk_distributor, fk_precision, geometry)
    SELECT 2016, 1.5, (SELECT id FROM qwat_vl.part_type WHERE value_fr = 'Point d''introduction'), 101, '{1}', 101, st_setsrid('point(2630000 1140000 0)'::geometry, 2056);
...
INSERT INTO qwat_od.valve (id, fk_district, fk_pressurezone, fk_distributor, fk_pipe,
    ...
    VALUES (1, 1, 1, 1, 1,
       ...
       st_setsrid('point(2530000 1140010 0)'::geometry,2056));
...
INSERT INTO qwat_od.pipe (id, fk_node_a, fk_node_b,
    ...
    VALUES (2, 1, 2, 101, 101, 101, 1, 101, 101, 101, 101,
       st_setsrid('linestring(2630000 1240000 0,2630000 1240010 0)'::geometry, 2056),
       2016, 101, True);
...
INSERT INTO qwat_od.vw_element_hydrant (year, fk_distributor, fk_status, fk_provider,
    ...
    VALUES (2016, '{1}', 101, 101, 
            ...
            12.3, 12.3, 12.3,
            '2016/01/01', 'sbe',
            st_setsrid('point(2630000 1240000 0)'::geometry,2056)
            );
...
INSERT INTO qwat_od.vw_element_installation (altitude, geometry, fk_distributor, installation_type, fk_status, fk_precision, fk_precisionalti, fk_watertype, fk_object_reference,
            name, year)
    VALUES (200, ST_SetSRID( ST_MakePoint( 2530050, 1138360, 0), 2056), '{1}', 'chamber', 101, 101, 101, 101, 101,
            'installation1', 2016);

-- create a cover
INSERT INTO qwat_od.cover (identification, form_dimension, fk_cover_type, circular, fk_installation, geometry, fk_precision)
    SELECT 'installation1', 15, 101, True, (SELECT id FROM qwat_od.vw_element_installation WHERE name = 'installation1'), ST_SetSRID( ST_MakePoint( 2530070, 1138370, 0), 2056), (SELECT id FROM qwat_vl.precision WHERE value_fr = 'Inconnu');
