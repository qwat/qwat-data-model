...
CREATE TABLE qwat_vl.status ( id integer not null, CONSTRAINT status_pk PRIMARY KEY (id) );
COMMENT ON TABLE qwat_vl.status IS 'Status table, with SIRE id.';

/* COLUMNS */
ALTER TABLE qwat_vl.status ADD COLUMN vl_active boolean default true; /* determines if the value list will be used or not */
ALTER TABLE qwat_vl.status ADD COLUMN value_en  varchar(30) default '';
ALTER TABLE qwat_vl.status ADD COLUMN value_fr  varchar(30) default '';
ALTER TABLE qwat_vl.status ADD COLUMN value_ro  varchar(30) default '';
...
ALTER TABLE qwat_vl.status ADD COLUMN helper_en text default '';
ALTER TABLE qwat_vl.status ADD COLUMN helper_fr text default '';
ALTER TABLE qwat_vl.status ADD COLUMN helper_ro text default '';

/* VALUES */

INSERT INTO qwat_vl.status (id, value_en, value_fr, value_ro, active, sire, helper_en, helper_fr, helper_ro) VALUES (1307, '', 'fictif', 'Fictiv', true, 1, '', 'L''objet n''existe pas mais est nécessaire à l''intégrité topologique.', 'Obiectul este fictiv dar este necesar pentru integritatea topologică.');    /*  */
...
INSERT INTO qwat_vl.status (id, value_en, value_fr, value_ro, active, sire, helper_en, helper_fr, helper_ro) VALUES (1305, '', 'détruit', 'Distrus', false, NULL, '', 'L''objet a été partiellement ou complètement détruit.', 'Obiectul este distrus complet sau parţial');    /*  */
INSERT INTO qwat_vl.status (id, value_en, value_fr, value_ro, active, sire, helper_en, helper_fr, helper_ro) VALUES (1302, '', 'hors service', 'Nefuncţional', false, NULL, '', 'Ne contribue actuellement pas au réseau, mais peut être remis en service sans opération spéciale. La mise hors service est considérée comme temporaire. Par exemple, une source mise hors service pour des problèmes de qualité.', 'Nu contribuie efectiv la reţea, dar poate fi repusă în stare funcţională fară operaţiuni speciale. Starea nefuncţională este considerată ca una temporară. De exemplu, putem avea o sursă care a fost pusă în stare nefuncţională datorită unor probleme de calitate.');    /*  */
INSERT INTO qwat_vl.status (id, value_en, value_fr, value_ro, active, sire, helper_en, helper_fr, helper_ro) VALUES (1301, '', 'en service', 'Funcţional', true, 1, '', 'Contribue actuellement au réseau.', 'Contribuie efectiv la reţea.');    /*  */
...
INSERT INTO qwat_vl.status (id, value_en, value_fr, value_ro, active, sire, helper_en, helper_fr, helper_ro) VALUES (1306, '', 'projet', 'Proiect', false, 0, '', 'L''objet est un projet validé en attente ou en cours de réalisation.', 'Obiectul face parte dintr-un proiect care se va face sau în curs de realizare. ');    /*  */
