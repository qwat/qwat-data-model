...
CREATE OR REPLACE FUNCTION qwat_sys.fn_installation_view_create(_installation_name text, _fields text[]) AS
$BODY$
	DECLARE 
	...
	BEGIN
		...
		EXECUTE format('
			...
			array_to_string(ff, ', ') FROM ( SELECT array_agg('i.'||f||' = NEW.'||f) AS f FROM unnest(main_fields) AS ff ) foo,
			'qwat_od.'||_installation_name,
			array_to_string(ff, ', ') FROM ( SELECT array_agg('j.'||f||' = NEW.'||f) AS f FROM unnest(_fields)     AS ff ) foo
		);
