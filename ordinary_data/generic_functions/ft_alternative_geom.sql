...
CREATE OR REPLACE FUNCTION qwat_od.ft_geometry_alternative_main() RETURNS TRIGGER AS
	$BODY$
	BEGIN
		IF NEW.geometry_alt1 IS NULL OR NEW.update_geometry_alt1 IS TRUE OR ( NEW.update_geometry_alt1 IS NULL AND ( TG_OP = 'INSERT' OR ST_Equals(OLD.geometry, OLD.geometry_alt1) ) ) THEN
			NEW.geometry_alt1 := NEW.geometry;
		END IF;
		IF NEW.geometry_alt2 IS NULL OR NEW.update_geometry_alt2 IS TRUE OR ( NEW.update_geometry_alt2 IS NULL AND ( TG_OP = 'INSERT' OR ST_Equals(OLD.geometry, OLD.geometry_alt2) ) ) THEN
