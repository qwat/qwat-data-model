...
CREATE OR REPLACE VIEW qwat_od.vw_pipe_child_parent AS
	...
	FROM (
		SELECT child,parent,
			...
			ST_LineInterpolatePoint(ST_MakeLine( start_point , end_point ),.5)::geometry(Point,:SRID) AS middle_point
		FROM (
			SELECT a.id AS child ,b.id AS parent, 
					ST_Force2d(ST_LineInterpolatePoint(a.geometry,.5))::geometry(Point,:SRID) AS start_point,
					/* select end_point at 4 meters from the closest side of the pipe */
					ST_ClosestPoint(ST_MakeLine(  
						ST_LineInterpolatePoint(ST_Force2D(b.geometry),   LEAST(1,  4/b._length2d/2))::geometry(Point,:SRID) ,
						ST_LineInterpolatePoint(ST_Force2D(b.geometry),GREATEST(0,1-4/b._length2d/2))::geometry(Point,:SRID) 
